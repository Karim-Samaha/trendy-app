...
const HomeScreen = () => {
  ...
  const fetchSections = async (order, count, name) => {
    try {
      const response = await axios.get(
        `${config.backendUrl}/homepage-sections?order=${order}`
      );
      setSections((prev) => ({
        ...prev,
        [count]: {
          productsList: response.data.data.subCategories[0]?.productList.filter(
            (item) => item?.active
          ),
          categoryName: name,
        },
  ...
  const fetchBanners = async () => {
    try {
      ...
      setImages((prev) => ({
        ...prev,
        banners: response.data.data.filter((item) => item.type === "BANNER"),
        heros: response.data.data.filter(
          (item) => item.type === "HERO_IMG" && item.active
        ),
      }));
      console.log("!!!!!!!!!");
  ...
  async function getHomePageSectionsSettings() {
    ...
    Promise.all([
      sectionsInfo.map((section, i) =>
        fetchSections(section?._id, i + 1, section?.name)
      ),
    ]);
    if (!res) {
  ...
  const scrollViewStyle = (list) => {
    if (list.length > 3) {
      ...
      return { flexDirection: "row", minWidth: "100%" };
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          {images?.heros.length > 0 ? (
            <SliderBox
              images={images.heros.map(
                (item) => `${config.backendBase}${item?.imageSrc}`
              )}
              onCurrentImagePressed={(i) =>
                handleBannersLinks(images.heros[i]?.route)
              }
              autoPlay
              circleLoop
              dotColor={"#13274F"}
          ...
          <View style={{ direction: "rtl" }}>
            <Text style={styles.ctgHeader}>التصنيفات</Text>
            <ScrollView
              horizontal
          ...
          <View>
            ...
            {images?.banners.length > 0 && (
              <>
                <Pressable
                  onPress={() => handleBannersLinks(images?.banners[0]?.route)}
                >
                  <Image
                    src={`${config.backendBase}${images?.banners[0].imageSrc}`}
          ...
          <View>
            ...
            {images?.banners.length > 1 && (
              <>
                <Pressable
                  onPress={() => handleBannersLinks(images?.banners[1]?.route)}
                >
                  <Image
                    src={`${config.backendBase}${images?.banners[1].imageSrc}`}
                    style={styles.bannerImg}
                  />
                </Pressable>
              </>
